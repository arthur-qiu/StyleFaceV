...
def save_videos(webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    """Save images to the disk.
    ...
    for label, im_data in visuals.items():
        if 'vid' not in label:
            ...
            video_name = '%s_%s.mp4' % (name, label)
            os.makedirs(image_dir.replace('/images','/videos'), exist_ok=True)
            img_save_path = os.path.join(image_dir, video_name)
            save_path = img_save_path.replace('/images/','/videos/')

            video = ((im_data.cpu() + 1.) / 2. * 255).type(torch.uint8).permute(0, 2, 3, 1)
            write_video(save_path,
                        video,
                        fps=15)
            # for i in range(len(video)):
            # for i in [0, 4, 9, 14, 19, 24, 29, 39, 44, 49, 59]:
            #     write_png(video[i].permute(2, 0, 1), img_save_path[:-4] + '_' + str(i) + '.png',
            #               compression_level=0)

            # video = tensor2ndarray255(im_data)
            # save_video(save_path, video)
...
def save_video(fname, images, output_fps=30, vcodec='libx264', filters=''):
    ...
    stream = ffmpeg.filter(stream, 'setpts', 'PTS')  # 2*PTS is for slower playback
    ...
    print('writing video to %s' % fname)
    for frame in images:
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    ...
                    if idx % ncols == 0:
                        ...
                        label_html_row = ''
                white_image = np.clip(np.ones_like(image_numpy.transpose([2, 0, 1])), 0, 1) * 255
                while idx % ncols != 0:
